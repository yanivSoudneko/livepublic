<template>
  <el-table :data="tableData" style="width: 100%">
    <el-table-column prop="stay.name" label="Stay" width="240"> </el-table-column>
    ...
    <el-table-column fixed="right" label="Operations" width="120">
      <template slot-scope="scope">
        <el-button @click="orderStatus(scope.$index, scope.row, 1)" type="text" size="small">Accept</el-button>
        <el-button @click="orderStatus(scope.$index, scope.row, 0)" type="text" size="small">Reject</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
...
export default {
  ...
  methods: {
    orderStatus(index, scope, status) {
      console.log('🚀 ~ file: backoffice-table.cmp.vue ~ line 35 ~ orderStatus ~ order', index, scope._id);
      console.log('isSame', scope === this.orders.find(o => o._id === scope._id));
      ...
      this.$store
        ...
        .catch(err => {
          console.error('🚀 ~ file: backoffice-table.cmp.vue ~ line 58 ~ .then ~ err', err);
        });
    },
    getOrders() {
      // debugger;
